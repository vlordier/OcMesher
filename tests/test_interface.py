...
"""Tests for ocmesher.utils.interface module."""
...
class TestAsInt:
    """Tests for AsInt conversion function."""

    def test_valid_int32_array(self, sample_numpy_int_array):
        result = AsInt(sample_numpy_int_array)
        # Result should be a ctypes pointer; verify it's not None and is usable
...
class TestAsDouble:
    """Tests for AsDouble conversion function."""
...
class TestAsFloat:
    """Tests for AsFloat conversion function."""
...
class TestAsBool:
    """Tests for AsBool conversion function."""
...
class TestRegisterFunc:
    """Tests for register_func helper."""

    def test_registers_function_on_object(self):
        obj = MagicMock()
        dll = MagicMock()
        mock_func = MagicMock()
        dll.my_func = mock_func
    ...
    def test_raises_on_missing_function(self):
        obj = MagicMock()
        dll = MagicMock(spec=[])  # no attributes
...
class TestLoadCdll:
    ...
    def test_raises_on_invalid_shared_library(self, tmp_path):
        import sys
        # Create a fake .so file that isn't actually a shared library
