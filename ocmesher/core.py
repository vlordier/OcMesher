...
class OcMesher:
    def __init__(
        ...
        for i in range(self.n_cameras):
            self.cameras[23 * i : 23 * (i + 1)] = np.concatenate(
                [np.linalg.inv(cam_poses[i])[:3, :4].reshape(-1), Ks[i].reshape(-1), [Hs[i]], [Ws[i]]]
        ...
        self.size = self.np_float_type(
            max(max(bounds[1] - bounds[0], bounds[3] - bounds[2]), bounds[5] - bounds[4]) * 1.1
        )
    ...
    def kernel_caller(
        ...
        XYZ_all: npt.NDArray[np.floating],
