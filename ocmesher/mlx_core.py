...
class MLXOcMesher:
    ...
    def _extract_meshes(self, kernels: list[Any]) -> MeshResult:
        ...
        with Timer("MLX construct mesh"):
            ...
            for e in range(n_elements):
                ...
                logger.info("element %d: %d vertices, %d faces", e, mesh.vertices.shape[0], mesh.faces.shape[0])
    ...
    def _evaluate_sdf(self, kernels: list[Any], positions: np.ndarray) -> np.ndarray:
        ...
        for start in range(0, n_points, chunk_size):
            ...
            with ThreadPoolExecutor(max_workers=self.n_sdf_workers) as executor:
                chunk_results = list(
                    executor.map(
                        lambda k_p: MLXOcMesher._evaluate_sdf_kernel(k_p[0], k_p[1]), [(k, chunk_pos) for k in kernels]
                    )
                )
    ...
    def _refine_surface_octree(
        ...
        for _ in range(max_iters):
            ...
            if corner_sdf is not None:
                ...
                child_mask, child_corner_sdf = self._find_surface_cubes(kernels, child_c, child_l)
    ...
    def _visibility_filter(self, positions: np.ndarray) -> np.ndarray:
        ...
        in_view_all = (
            (depth > 0) & (px >= 0) & (px < w_all[:, np.newaxis]) & (py >= 0) & (py < h_all[:, np.newaxis])
        )  # (C, N)
